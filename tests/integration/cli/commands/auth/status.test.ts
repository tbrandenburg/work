...
describe('Auth Status Command Integration', () => {
  ...
  it('should show auth status in table format', () => {
    const result = execSync(`node ${binPath} auth status`, {
      encoding: 'utf8',
    });
  ...
  it('should handle error when getting auth status', () => {
    ...
    expect(() => {
      execSync(`node ${binPath} auth status nonexistent`, {
        encoding: 'utf8',
        stdio: 'pipe',
      });
    }).toThrow();
  ...
  it('should handle invalid format option', () => {
    expect(() => {
      execSync(`node ${binPath} auth status --format invalid`, {
        stdio: 'pipe',
      });
    }).toThrow();
  });
  ...
  it('should handle context argument branch', () => {
    ...
    expect(() => {
      execSync(`node ${binPath} auth status nonexistent-context`, {
        encoding: 'utf8',
        stdio: 'pipe',
      });
    }).toThrow();
