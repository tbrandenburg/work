...
describe('Work Item Lifecycle E2E', () => {
  ...
  it('should complete create → close → reopen workflow', () => {
    ...
    const getOutput = execSync(`node ${binPath} get ${workItemId}`, { encoding: 'utf8' });
    expect(getOutput).toContain('State:       new');
    
    // Close work item
    const closeOutput = execSync(`node ${binPath} close ${workItemId}`, { encoding: 'utf8' });
    expect(closeOutput).toContain('Closed task');
    
    // Verify state is 'closed'
    const getClosedOutput = execSync(`node ${binPath} get ${workItemId}`, { encoding: 'utf8' });
    ...
    const getFinalOutput = execSync(`node ${binPath} get ${workItemId}`, { encoding: 'utf8' });
