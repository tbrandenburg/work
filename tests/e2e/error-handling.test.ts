...
describe('Error Handling E2E', () => {
  ...
  describe('Work Item Errors', () => {
    it('should handle non-existent work item operations', () => {
      ...
      expect(() => {
        execSync(`node ${binPath} edit NONEXISTENT --title "test"`, { stdio: 'pipe' });
      }).toThrow();
    });
    ...
    it('should handle reopen non-closed work item gracefully', () => {
      ...
      const createOutput = execSync(`node ${binPath} create "Test Task"`, { encoding: 'utf8' });
      const workItemId = createOutput.match(/TASK-\d+/)?.[0];
      
      // Reopen already non-closed work item (should succeed - idempotent)
      const reopenOutput = execSync(`node ${binPath} reopen ${workItemId}`, { encoding: 'utf8' });
  ...
  describe('Context Errors', () => {
    it('should handle non-existent context operations', () => {
      ...
      expect(() => {
        execSync(`node ${binPath} context remove nonexistent`, { stdio: 'pipe' });
      }).toThrow();
    });
  });
  ...
  describe('Relation Errors', () => {
    it('should handle invalid relation operations gracefully', () => {
      ...
      expect(() => {
        execSync(`node ${binPath} link ${workItemId} NONEXISTENT --type parent_of`, { stdio: 'pipe' });
      }).toThrow();
      
      // Try to unlink non-existent relation (should succeed - idempotent)
      const unlinkOutput = execSync(`node ${binPath} unlink ${workItemId} NONEXISTENT --type parent_of`, { encoding: 'utf8' });
