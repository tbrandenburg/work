...
describe('Relation Lifecycle E2E', () => {
  ...
  it('should complete link → unlink workflow', () => {
    ...
    const createOutput1 = execSync(`node ${binPath} create "Parent Task"`, { encoding: 'utf8' });
    const parentId = createOutput1.match(/TASK-\d+/)?.[0];
    expect(parentId).toBeDefined();
    
    const createOutput2 = execSync(`node ${binPath} create "Child Task"`, { encoding: 'utf8' });
    const childId = createOutput2.match(/TASK-\d+/)?.[0];
    expect(childId).toBeDefined();
    
    // Link work items (parent_of relation)
    const linkOutput = execSync(`node ${binPath} link ${parentId} ${childId} --type parent_of`, { encoding: 'utf8' });
    ...
    // Unlink work items
    const unlinkOutput = execSync(`node ${binPath} unlink ${parentId} ${childId} --type parent_of`, { encoding: 'utf8' });
