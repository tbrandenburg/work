...
describe('Context Lifecycle E2E', () => {
  ...
  it('should handle context commands without persistence', () => {
    ...
    const addOutput = execSync(`node ${binPath} context add test-ctx --tool local-fs --path ./work-items`, { encoding: 'utf8' });
    expect(addOutput).toContain('Added context');
    
    // Test context list (should show no contexts due to no persistence)
    const listOutput = execSync(`node ${binPath} context list`, { encoding: 'utf8' });
