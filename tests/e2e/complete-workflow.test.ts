...
describe('Complete Workflow E2E', () => {
  ...
  it('should complete full work item management journey', () => {
    ...
    const createOutput1 = execSync(`node ${binPath} create "Epic: User Authentication"`, { encoding: 'utf8' });
    const epicId = createOutput1.match(/TASK-\d+/)?.[0];
    
    const createOutput2 = execSync(`node ${binPath} create "Implement login form"`, { encoding: 'utf8' });
    const taskId = createOutput2.match(/TASK-\d+/)?.[0];
    
    // Edit work items with details
    execSync(`node ${binPath} edit ${epicId} --description "Complete user authentication system"`, { stdio: 'pipe' });
    execSync(`node ${binPath} edit ${taskId} --assignee "developer@example.com"`, { stdio: 'pipe' });
    
    // Create relationships
    execSync(`node ${binPath} link ${epicId} ${taskId} --type parent_of`, { stdio: 'pipe' });
    
    // Work on task
    execSync(`node ${binPath} start ${taskId}`, { stdio: 'pipe' });
    
    // Add comments (placeholder command)
    execSync(`node ${binPath} comment ${taskId} "Started implementation"`, { stdio: 'pipe' });
    ...
    // Verify final state
    const getTaskOutput = execSync(`node ${binPath} get ${taskId}`, { encoding: 'utf8' });
