...
describe('JSON Output Validation', () => {
  describe('Structured Response Format', () => {
    ...
    it('should output structured JSON for hello command', () => {
      const result = execSync(
        `node ${binPath} hello world --from test --format json`,
        { encoding: 'utf8' }
      );
      const parsed = JSON.parse(result);
  ...
  describe('State Change Commands JSON Output', () => {
    ...
    beforeAll(() => {
      ...
      const result = execSync(
        `node ${binPath} create "State change test task" --format json`,
        { encoding: 'utf8' }
      );
      const parsed = JSON.parse(result);
    ...
    it('should output structured JSON for start command', () => {
      const result = execSync(`node ${binPath} start ${taskId} --format json`, {
        encoding: 'utf8',
      });
      const parsed = JSON.parse(result);
    ...
    it('should output structured JSON for close command', () => {
      const result = execSync(`node ${binPath} close ${taskId} --format json`, {
        encoding: 'utf8',
      });
      const parsed = JSON.parse(result);
    ...
    it('should output structured JSON for reopen command', () => {
      const result = execSync(
        `node ${binPath} reopen ${taskId} --format json`,
        { encoding: 'utf8' }
      );
      const parsed = JSON.parse(result);
  ...
  describe('Edge Cases', () => {
    it('should handle empty data responses', () => {
      ...
      const result = execSync(
        `node ${binPath} list where "state=nonexistent" --format json`,
        { encoding: 'utf8' }
      );
      const parsed = JSON.parse(result);
    ...
    it('should handle unicode characters in JSON output', () => {
      const result = execSync(
        `node ${binPath} create "Test with émojis 🚀" --format json`,
        { encoding: 'utf8' }
      );
      const parsed = JSON.parse(result);
