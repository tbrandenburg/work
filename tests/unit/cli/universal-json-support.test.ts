...
describe('Universal JSON Output Support', () => {
  describe('All Commands JSON Support', () => {
    const commands = [
      ...
      { cmd: 'close TASK-001', expectData: true, setup: 'create "Close task" && start TASK-001' },
      { cmd: 'reopen TASK-001', expectData: true, setup: 'create "Reopen task" && start TASK-001 && close TASK-001' },
      
      // Property commands
      { cmd: 'set TASK-001 --assignee "test"', expectData: true, setup: 'create "Set task"' },
      { cmd: 'unset TASK-001 --assignee', expectData: true, setup: 'create "Unset task"' },
      { cmd: 'edit TASK-001 --title "Updated"', expectData: true, setup: 'create "Edit task"' },
      
      // Relation commands
      { cmd: 'link TASK-001 TASK-002 --type blocks', expectData: true, setup: 'create "Link1" && create "Link2"' },
      { cmd: 'unlink TASK-001 TASK-002 --type blocks', expectData: true, setup: 'create "Unlink1" && create "Unlink2" && link TASK-001 TASK-002 --type blocks' },
      
      // Utility commands
      { cmd: 'hello --from test', expectData: true },
      { cmd: 'comment TASK-001 "Test comment"', expectData: true, setup: 'create "Comment task"' },
      { cmd: 'delete TASK-001', expectData: true, setup: 'create "Delete task"' },
      { cmd: 'move TASK-001 other-context', expectData: true, setup: 'create "Move task"' },
        ...
        try {
          ...
          if (setup) {
            ...
            setupCommands.forEach(setupCmd => {
              try {
                execSync(`node ${binPath} ${setupCmd}`, { encoding: 'utf8', stdio: 'pipe' });
              } catch (error) {
          ...
          if (error instanceof Error && error.message.includes('SyntaxError')) {
            throw new Error(`Command "${cmd}" did not output valid JSON: ${error.message}`);
          }
        }
      });
    });
  ...
  describe('JSON Structure Validation', () => {
    it('should have consistent response structure across commands', () => {
      ...
      commands.forEach(cmd => {
        const result = execSync(`node ${binPath} ${cmd} --format json`, { encoding: 'utf8' });
        const parsed = JSON.parse(result);
  ...
  describe('Edge Cases', () => {
    it('should handle empty data responses', () => {
      const result = execSync(`node ${binPath} context list --format json`, { encoding: 'utf8' });
      const parsed = JSON.parse(result);
