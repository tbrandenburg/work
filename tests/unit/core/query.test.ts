...
describe('Query System', () => {
  ...
  describe('Error handling and edge cases', () => {
    ...
    it('should handle unsupported operators in evaluation', () => {
      ...
      expect(() => executeQuery(mockItems, malformedQuery)).toThrow(
    ...
    it('should handle complex nested expressions', () => {
      const query = parseQuery(
        'where (state=active OR state=new) AND priority>=high'
      );
    ...
    it('should handle quoted strings with special characters', () => {
      ...
      const query = parseQuery(
        'where title="Task with spaces and symbols: @#$%"'
      );
  ...
  describe('Performance and coverage', () => {
    it('should handle large numbers of conditions efficiently', () => {
      ...
      const complexQuery =
        'where (priority=high OR priority=critical) AND state!=closed AND createdAt>2024-01-01 AND NOT assignee=""';
