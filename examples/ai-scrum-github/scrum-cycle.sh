...
if [ "$VERBOSE" = "true" ]; then
    ...
    echo "⚙️ work CLI notifies Reviewer agent to review task completion"
    # Get only the most recent 3 closed tasks for review (to avoid reviewing old issues)
    RECENT_CLOSED_TASKS=$(work list --format json | jq -c '{timestamp: now | strftime("%Y-%m-%dT%H:%M:%SZ"), itemCount: (.data | length), items: [.data[] | select(.state == "closed")][0:3]}')
    if [ "$RECENT_CLOSED_TASKS" != '{"timestamp":"","itemCount":0,"items":[]}' ]; then
        RECENT_COUNT=$(echo "$RECENT_CLOSED_TASKS" | jq '.items | length')
        if [ "$RECENT_COUNT" != "0" ]; then
            AGENT_OUTPUT=$(echo "$RECENT_CLOSED_TASKS" | ./reviewer.sh 2>&1 | grep "^  " | sed 's/^  /  /' | sed 's/^/[Reviewer] /')
            if [ -n "$AGENT_OUTPUT" ]; then
                echo "$AGENT_OUTPUT"
            fi
        fi
